...
const API = "https://sniplink-backend-production.up.railway.app/login";
...
export default function App() {
  ...
  const fetchUrls = async (a = auth) => {
    if (!a) return;
    try {
      const res = await fetch(`${API}/url/allurl`, {
        headers: { Authorization: getAuth(a.username, a.password) },
      });
  ...
  const handleShorten = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API}/url/short-url`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: getAuth(auth.username, auth.password),
        },
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`${API}/url/delete-url/${id}`, {
        method: "DELETE",
        headers: { Authorization: getAuth(auth.username, auth.password) },
      });
  ...
  return (
    <div style={styles.root}>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ ...styles.toast, background: toast.type === "error" ? "#ff4d6d" : "#06d6a0" }}>
          {toast.msg}
        </div>
      )}
      ...
      <nav style={styles.nav}>
        <span style={styles.logo} onClick={() => setPage(auth ? "dashboard" : "home")}>
          ⚡ Sniplink
        </span>
        <div style={styles.navLinks}>
          {auth ? (
            <>
              <span style={styles.navUser}>👤 {auth.username}</span>
              <button style={styles.navBtn} onClick={() => { setAuth(null); setPage("home"); setUrls([]); }}>
                Logout
              </button>
            </>
          ) : (
            <>
              ...
              <button style={{ ...styles.navBtn, ...styles.navBtnPrimary }} onClick={() => setPage("register")}>
                Sign Up
              </button>
            </>
          )}
      ...
      {page === "home" && (
        <div style={styles.hero}>
          ...
          <div style={styles.heroStats}>
            <div style={styles.stat}><span style={styles.statNum}>∞</span><span style={styles.statLabel}>URLs</span></div>
            <div style={styles.statDivider} />
            <div style={styles.stat}><span style={styles.statNum}>1ms</span><span style={styles.statLabel}>Redirect</span></div>
            <div style={styles.statDivider} />
            <div style={styles.stat}><span style={styles.statNum}>100%</span><span style={styles.statLabel}>Free</span></div>
          </div>
        </div>
      )}
      ...
      {page === "login" && (
        <div style={styles.formWrap}>
          <div style={styles.card}>
            ...
            <div style={styles.oauthDivider}><span>or continue with</span></div>
            <div style={styles.oauthBtns}>
              <a href={`${API}/oauth2/authorization/google`} style={styles.oauthBtn}>
              ...
              <a href={`${API}/oauth2/authorization/github`} style={styles.oauthBtn}>
            ...
            <p style={styles.switchText}>
              No account? <span style={styles.link} onClick={() => setPage("register")}>Sign up</span>
            </p>
          </div>
        </div>
      )}
      ...
      {page === "register" && (
        <div style={styles.formWrap}>
          <div style={styles.card}>
            ...
            <p style={styles.switchText}>
              Already have an account? <span style={styles.link} onClick={() => setPage("login")}>Sign in</span>
            </p>
          </div>
        </div>
      )}
        ...
        <div style={styles.dashboard}>
          ...
          <div style={styles.shortenerBox}>
            <div style={styles.shortenerInner}>
              ...
              />
              <button style={styles.shortenerBtn} onClick={handleShorten} disabled={loading}>
                {loading ? "..." : "Shorten ⚡"}
              </button>
            </div>
          </div>
          ...
          {result && (
            <div style={styles.resultBox}>
              ...
              <a href={shortLink(result.shorturl)} target="_blank" rel="noreferrer" style={styles.resultLink}>
                ...
                <div key={url.id} style={styles.urlCard}>
                  <div style={styles.urlCardLeft}>
                    <div style={styles.urlShort}>
                      <a href={shortLink(url.shorturl)} target="_blank" rel="noreferrer" style={styles.urlShortLink}>
...
const styles = {
  ...
  heroTitle: {
    ...
    letterSpacing: "-2px", color: "#f0f0ff",
  },
  accent: {
    background: "linear-gradient(135deg, #818cf8, #c084fc)",
    WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
  ...
  btnPrimary: {
    ...
    letterSpacing: "-0.3px", width: "100%", marginTop: "8px",
  ...
  statLabel: { fontSize: "12px", color: "#6060a0", textTransform: "uppercase", letterSpacing: "1px" },
  ...
  cardIcon: { fontSize: "40px", textAlign: "center" },
  ...
  oauthDivider: {
    ...
    "::before": { content: '""', flex: 1, height: "1px", background: "rgba(255,255,255,0.08)" },
    "::after": { content: '""', flex: 1, height: "1px", background: "rgba(255,255,255,0.08)" },
  },
  oauthBtns: { display: "flex", gap: "12px" },
  oauthBtn: {
    flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: "8px",
    padding: "12px", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.1)",
    background: "rgba(255,255,255,0.04)", color: "#c0c0e0", fontSize: "14px",
  ...
  urlShortLink: {
    color: "#818cf8", fontWeight: 700, fontSize: "15px",
    textDecoration: "none",
  },
  copySmall: {
  ...
  deleteBtn: {
    background: "rgba(255,77,109,0.1)", border: "none",
    borderRadius: "10px", padding: "10px 14px",
    cursor: "pointer", fontSize: "16px",
    color: "#ff4d6d", marginLeft: "16px",
